...
```
![进程关系](%E8%BF%9B%E7%A8%8B%E5%85%B3%E7%B3%BB.png)
## 相关函数
```
◼ pid_t getpgrp(void);//获取当前进程的进程组id
◼ pid_t getpgid(pid_t pid);//获取传入的pid的进程的进程组id
◼ int setpgid(pid_t pid, pid_t pgid);//设置pid的进程组为pgid
◼ pid_t getsid(pid_t pid);//获取pid的会话id
◼ pid_t setsid(void);//设置pid的会话id
```
## 守护进程
```
