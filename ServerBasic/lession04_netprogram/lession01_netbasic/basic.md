# 基础
## IP协议长度4个字节(255.255.255.255)，MAC协议长度6个字节(ff:ff:ff:ff:ff:ff)
## 网络架构
### C/S结构
```
...
连接。每一个网卡都有一个被称为 MAC 地址的独一无二的 48 位串行号。网卡的主要功能：
1.数据的封装与解封装、
2.链路管理、
3.数据编码与译码
...
A 类 IP 地址的子网掩码为 255.0.0.0，每个网络支持的最大主机数为 256 的 3 次方 - 2 = 16777214 台。B类IP地址
...
网络正常工作
ip地址显示为192.168.100.10/22 则表示其子网掩码前面为1得个数为22个，
```
## 端口
```
...
```
### TCP协议
![TCP协议](TCP%E5%A4%B4%E9%83%A8.png)
```
1. 源端口号：发送方端口号
2. 目的端口号：接收方端口号
...
1 就表示这是一个连接请求或连接接受报文12. 终止 FIN：用来释放一个连接。当 FIN = 1 时，表明此报文段的发送方的数据已经发送完毕，并要
求释放运输连接
...
```
### IP协议
![IP协议头](IP%E5%8D%8F%E8%AE%AE%E5%A4%B4.png)
```
